...
- The lm model is statistically more significant than before, with p values less than 0.05 for the intercept and treatment. The residuals are closer to normal as seen in the normality of residuals. 
- The Poisson regression model is similar to the Poisson model that was performed for the data up to 2018. The p values are significant for both the intercept and treat. The reduction in deviance from the null model to the fitted model is very small, indicating that the Poisson model doesn’t improve the fit significantly, suggesting overdispersion or misspecification of the model. 
- The Negative binomial regression model is statistically more significant than before, with p values less than 0.05 for intercept and treatment. The posterior predictive check observed data fits perfectly to the model predictive data and the uniformity of residuals dots fall perfectly along the line. This model passed the overdispersion test and did ok in the zero inflation test. 
...
- Now, looking at the data up to 2024 as a Negative Binomial regression model estimating how the counts outcome, as number of events or occurrences, changes in relation to both the treatment and the yearly time periods from 2013 (base year is 2012) to 2024, as well as the interaction between treatment and year. It becomes more clear that though the MPA region did increase counts in lobster from 2018 to 2021, by 2023 it is close to even with non MPA regions. This is probably due to the spillover effects previously discussed. For the most part there seems to be a trend in increase and decrease for MPA and non MPA regions suggesting that MPA regions do not positively affect counts. By 2024 non MPA regions have a much smaller count than MPA regions but I would expect this to change and follow the same pattern seen in the previous years. 
- Having more years to look over changes in patterns is quite helpful, and the statistical models performed demonstrated that by improving residuals. The graph below is better at representing the difference between MPA and non MPA regions counts in lobster. Considering this is only 12 years, it would be good to continue to monitor the pattern and not jump to conclusions. Bias can be very much present and the variables considered are not specific enough. For instance, changes in habitat quality, climate conditions, fishing regulations, and predator-prey relationships could all play a role. Incorporating more specific covariates like environmental factors and fishing pressure into the model might help reduce bias and improve the model’s robustness. Interactions are much more likely between variables and could better help address the bias that may be present.
